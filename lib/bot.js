...
class Bot {
  constructor(options) {
    ...
    this.alternateParallelPingFix = options.alternateParallelPingFix === true; // default: false
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (this.alternateParallelPingFix) {
        // Encode A-Z,a-z,0-9 to the Unicode block 0x1d400 (Mathematical Alphanumeric Symbols.)
        // This only works for nicks that contain those characters.
        displayUsername = displayUsername.split('').map(c => c.codePointAt(0)).map(n => String.fromCodePoint(n >= 64 && n <= 90 ? n - 65 + 0x1d400 : n >= 97 && n <= 122 ? n - 97 + 0x1d41a : n >= 48 && n <= 57 ? n - 48 + 0x1d7e2 : n )).join('');
      }
      else if (this.parallelPingFix) {
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (webhook) {
      ...
      webhook.client.send({
        ...
        allowedMentions: { parse: canPingEveryone ? [ 'everyone', 'roles', 'users' ] : [ 'users' ] }
      }).catch(logger.error);
      return;
    }
